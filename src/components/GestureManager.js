...
export const GestureManager = ({ cx, cy, nodeValue, infoIndex, radius, isInsideCircle }) => {
  ...
  const currentIndexRef = useRef(infoIndex); // Track the current index for TTS
  ...
  const handleSecondTouch = (e) => {
    if (e.touches.length === 2 && firstTouchRef.current) {
      ...
      if (isInsideCircle(firstTouchRef.current.clientX, firstTouchRef.current.clientY) && distance <= 200 && lastSecondTapRef.current !== secondTouch.identifier) {
        // Announce the value of the current index in nodeValue
        speakValue(nodeValue[currentIndexRef.current]); 

        // Update the index for the next tap, cycling through nodeValue
        currentIndexRef.current = (currentIndexRef.current + 1) % nodeValue.length; // Cycle through indices
  ...
  const handleTouchEnd = (e) => {
    ...
    if (e.touches.length === 0) {
      ...
      lastSecondTapRef.current = null; // Reset second tap tracking
      currentIndexRef.current = infoIndex; // Reset the index to the initial value
    }
  };
