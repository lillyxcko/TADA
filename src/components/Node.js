...
const Node = ({ id, cx, cy, r, pitch, value, links }) => {
  ...
  const isInsideCircle = useCallback(
    (touchX, touchY) => {
      ...
      const distanceSquared =
        (touchX - centerX) ** 2 + (touchY - centerY) ** 2;
  ...
  const handleNodeTouchMove = useCallback(
    (e) => {
      e.preventDefault();
      gestureManager.handleTouchMove(e); // Add this line
  ...
  const handleNodeTouchEnd = useCallback(
    (e) => {
      ...
      gestureManager.handleTouchEnd(e); // Pass the event
    },
    [id, r, gestureManager]
  );
    ...
    <>
      <circle
        ...
        style={{
          ...
          touchAction: 'none', // Added to ensure touch events are captured
        }}
      />
